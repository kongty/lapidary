...
class SubAccelerator(maestro.SubAccelerator):
    def __init__(self, num_pe, l1_size, l2_size, noc_bw, offchip_bw):
        self.num_pe = num_pe
        self.l1_size = l1_size
        self.l2_size = l2_size
        self.noc_bw = noc_bw
        self.offchip_bw = offchip_bw
        super().__init__(self.num_pe, 1, int(self.l1_size/self.num_pe),
                         self.l2_size, self.offchip_bw, [self.noc_bw, self.noc_bw], [1, 1], [True, True])
...
class CostModel:
    def __init__(self,  accelerator: Accelerator, energy_table: EnergyTable):
        self.energy_table = energy_table

    def _run(self, subaccelerator: SubAccelerator, layer: Layer, dataflow: Dataflow) -> CostResult:
        """
        Run simulation give the dataflow.
        """
        ...
        cost_result = CostResult(latency=latency, energy=energy, edp=latency*energy, l1_size=l1_size, l2_size=l2_size,
                                 l1_energy=l1_energy, l2_energy=l2_energy, offchip_bw=result['offchip_bw'])
        return cost_result

    def sweep(self, accelerator: Accelerator, nn: Network, metric: Metric):
